  ...
  holiday_hash.collect do |season, holidays| #season = individual seasons, holidays = hash of one season's holidays
    ...
    holidays.collect do |holiday, supplies| #holiday = individual holidays, supplies = array of supplies
      if holiday == :new_years || holiday == :fourth_of_july || holiday == :memorial_day
        holiday_array << (holiday.to_s).split("_").each {|word| holiday_array << word}  #turn holiday from symbol to string, split the two words into separate strings, put into array
